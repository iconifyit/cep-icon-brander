...
// ****************** MIXED PREVIEW *******************
// Artboard name : preview@1000x3750
// ****************************************************

rules.add({
    TARGET : 'preview@1000x3750',
    GUTTER : 0,
    COLS   : 6,
    ROWS   : 4,
    SCALE  : 150,
    ...
    onLoad : function(fileList) {
        ...
        this.ROWS = Math.ceil(fileList.length / this.COLS);
    ...
    onFinish : function(theIcons) {
        ...
        try {
            ...
            var bg1 = new Background({
                fillColor : new RGB(232, 242, 218),
                top       : bounds[1],
                left      : bounds[0],
                width     : width,
                height    : height,
                name      : 'bg1-' + this.getTarget()
            });
            ...
            var bg3 = new Background({
                fillColor : new RGB(247, 237, 249),
                top       : bounds[1] - 1500,
                left      : bounds[0],
                width     : width,
                height    : height,
                name      : 'bg3-' + this.getTarget()
            });
            ...
            var bg5 = new Background({
                fillColor : new RGB(229, 246, 249),
                top       : bounds[1] - 3000,
                left      : bounds[0],
                width     : width,
                height    : height,
                name      : 'bg5-' + this.getTarget()
            });
            ...
            Host.logger.error(
                'Error adding colored backgrounds to {target}', 
        ...
        // Move the icons to the second panel.
                    
        try {
            for (var i = 0; i < theIcons.length; i++) {            
                try {
                    theIcons[i].select = true;
                }
                catch(e) {
                    Host.logger.error(
                        '{target} move error : {err}', 
			...
			theGroup.position = [
				((rect[2] - rect[0]) - (theGroup.width)) / 2,
				((750 - theGroup.height) / 2) * -1
			];
        ...
        try {
			...
			var target = bg1;     // doc.pathItems[4339];
        ...
        // Add outlined preview
        
        try {
            
			theOutline.selected = true;
			app.executeMenuCommand('copy');
			app.executeMenuCommand('pasteFront');
			...
			var source = theItem; // doc.groupItems[284];
			var target = bg3;     // doc.pathItems[4339];
